...
function buildProjectOwnerThreadPrompt({ project, context, ownerThread }) {
  ...
  return [
    `你现在是「${project.title}」的项目负责人 Thread，同时也是该项目的执行 Agent。`,
    "",
    "你的身份和职责：",
    ...
    "6. 你可以领取并执行本项目任务；完成后提交 deliver-task 等待总项目经理验收，不要自己 accept-delivery。",
    "",
    "项目资料：",
    `- 项目 ID：${project.id}`,
    `- GitHub 仓库：${githubUrl}`,
    `- 控制中心路径：${DEFAULT_CONTROL_CENTER_PATH}`,
    "- 线上 Dashboard 数据源：121.40.207.7:/opt/dashboard/data/workspace",
    `- 你的负责人 Thread 标识：${threadId}`,
    `- 你的显示名称：${threadName}`,
    "",
    "对接方式：",
    "- 优先把状态写到线上服务器，因为浏览器 Dashboard 读取的是 121.40.207.7 的数据。",
    "- 如果当前环境可以直接运行 `dashboard-ccc`，就直接运行；否则用 `ssh root@121.40.207.7 'dashboard-ccc ...'`。",
    "- 本地开发数据只作为辅助，不要只写本地而不写线上。",
    "",
    "首次接入时先执行：",
    "",
    "ssh root@121.40.207.7 'dashboard-ccc set-project-owner \\",
    `  --project ${project.id} \\`,
    `  --thread ${threadId} \\`,
    `  --name "${threadName}" \\`,
    "  --assigned-by yyg \\",
    `  --note "负责 ${project.id} 项目状态、需求上下文、风险、任务建议和任务执行"'`,
    "",
    "如果你在本地控制中心环境，也可以同步执行：",
    "",
    ...
    "",
    "然后提交负责人报告。报告里的 --context-summary、--p0、--p1、--p2 会成为 Dashboard 中优先展示的项目上下文口径。只有当你确信项目需要新工作时，才添加 --proposed-task：",
    "",
    "ssh root@121.40.207.7 'dashboard-ccc owner-report \\",
    `  --project ${project.id} \\`,
    `  --thread ${threadId} \\`,
    ...
    `  --project ${project.id} \\`,
    `  --agent ${threadId} \\`,
    "  --acceptance-note \"我已自动领取最高优先级可执行任务，并理解当前项目上下文。\" \\",
    "  --plan \"读取任务执行包，完成实现与验证，然后提交 deliver-task 等待总项目经理验收。\" \\",
    "  --eta \"下一次自动检查前更新状态或提交交付\"'",
    "",
    "领取成功后，从命令输出或 show-project-dashboard 中取得 task id，然后执行：",
    "",
    "ssh root@121.40.207.7 'dashboard-ccc start-task \\",
    `  --project ${project.id} \\`,
    "  --task <task-id> \\",
    `  --agent ${threadId}'`,
    "",
